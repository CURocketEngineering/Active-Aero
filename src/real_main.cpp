...
//     previousPredictedApogee = ApogeePrediction::newPredictApogee(KF.x_hat[1], telemData.sensorData["altitude"].altitude, telemData.sensorData["pressure"].pressure, telemData.sensorData["temperature"].temperature, dragCoefficent, rocketMass, crossArea);
//     printf("%i\t%d", 0, 0);
//   }
...
//         if (startCoastTime - currentTime > 1200) {
//             if (telemData.sensorData["altitude"].altitude < desiredApogee - 200) {
//                 double max = 0;
//                 int setting = 0;
//                 for (int i = 0; i < numAngles; ++i) {
//                     previousPredictedApogee = ApogeePrediction::predictApogeeWithFlaps(KF.x_hat[1], telemData.sensorData["altitude"].altitude, telemData.sensorData["pressure"].pressure, telemData.sensorData["temperature"].temperature, dragCoefficent, rocketMass, crossArea, areas[i]);
...
//                     }
//                 }
//                 ms24.setAngle(angles[setting]);
//                 printf("%i\t%d", angles[setting], max);
//             }
//             else {
//                 double min = 10000;
//                 int setting = 0;
...
//         }
//     }
//         //if time or altitude < threshold                       //allow some overshooting
//             //if desired apogee > predicted apogee              //iterate from start
//                 //for (int i = 0; i < numAngles; ++i)
//                     //predict apogee with inputs until one is close to the upperbound
//             //else desired apogee <= predicted apogee           //iterate from end
//                 //for (int i = numAngles - 1; i >= 0; --i)
//                     //predict apogee with inputs until one is close to the upperbound
        
        
//         //else time or altitude >= threshold                    //aim for apogee
//             //if desired apogee > predicted apogee              //iterate from start
//                 //for (int i = 0; i < numAngles; ++i)
//                     //predict apogee with inputs until one is close to desired apogee
//             //else desired apogee <= predicted apogee           //iterate from end
//                 //for (int i = numAngles - 1; i >= 0; --i)
//                     //predict apogee with inputs until one is close to desired apogee
//   }
